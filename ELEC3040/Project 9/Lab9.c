...
static unsigned int ccr_value [11] = {        //CCRy values according to button press
    ...
    2399, 2799, 3199, 3599, 4001

};

static unsigned int store_arr [100];					//Used to store the values specified in Lab9
...
static void PinSetup1() {
    ...
    GPIOA->PUPDR |= (0x00000550);        //pull-up// row, PA[5:2] = 01
		...
		GPIOA->MODER &= ~(GPIO_MODER_MODER1);		//Setting up PA1
...
/*#########################################################*/
/* Lab9 -- function used to store the value of ADC */
void Store_value() {		//Use store_arr[]
		//Short delay of approx 0.5 seconds
		int o,p,q,val_counter;
		for (o=0; o<20; o++) { //outer loop
      for (p=0; p<8225; p++) { //inner loop
         q = p; //dummy operation for single-step test
      } //do nothing
   }
		
	 if (val_counter >= 100) {
			val_counter = val_counter - 100;
			//Do you stop when we get 100? What is supposed to happen?
	 }
	 store_arr[val_counter] = adc_in;
	 val_counter++;
}
/*##########################################################*/
...
static void keypad() {
    ...
    GPIOB->ODR |= button;        //output button value, PB[6:3]
		...
		/*######################################*/
		/* Lab 9 - store current ADC into array */
		Store_value();	
...
int main(void) {
    ...
    PulseSetup();
		...
		adc_in = ADC1->DR;						//Assign data register value to adc_in.
